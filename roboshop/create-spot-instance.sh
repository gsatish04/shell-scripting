...
CREATE() {
 ...
 if [ $COUNT -eq 0 ]; then
  aws ec2 run-instances --launch-template LaunchTemplateId=lt-088ac5479d3b047bb,Version=2 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$1}]" "ResourceType=spot-instances-request,Tags=[{Key=Name,Value=$1}]"| jq &>/dev/null
 else
 ...
 aws route53 change-resource-record-sets --hosted-zone-id Z0577679A6W027W027W86RBE --change-batch file:///tmp/record.json | jq &>/dev/null
}
